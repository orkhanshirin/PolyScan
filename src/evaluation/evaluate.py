...
def evaluate_model(config: dict[str, Any]) -> dict[str, float]:
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="evaluating"):
            ...
            if 'dice' in config['metrics']:
                metrics['dice'].append(dice_score(preds, mask, config['model_params']['num_classes']))
            
            if 'iou' in config['metrics']:
                metrics['iou'].append(iou_score(preds, mask, config['model_params']['num_classes']))
            
            if 'sensitivity' in config['metrics'] or 'specificity' in config['metrics']:
                sens, spec = sensitivity_specificity(preds, mask, config['model_params']['num_classes'])
                metrics['sensitivity'].append(sens)
                metrics['specificity'].append(spec)
            
            if config.get('compute_hd95'):
                # convert to one-hot format for hd95
                hd_mask = F.one_hot(mask, config['model_params']['num_classes']).permute(0, 3, 1, 2)
                metrics['hd95'].append(
                    hausdorff_distance_95(probs, hd_mask.float(), config['model_params']['num_classes'])
                )
    ...
    if config['visualization']['enable']:
        ...
        overlays = [overlay_mask(img, pred) 
                   for img, pred in zip(images, predictions)]
        
        plot_predictions(
            images=images[:config['visualization']['num_samples']],
            masks=[m.squeeze() for m in ground_truths[:config['visualization']['num_samples']]],
